...
// Edited by Whistle for Bombrush
_education = paramsarray select 17;
_candie = if (paramsarray select 18 == 1) then {true} else {false};
_bleedouttime = paramsarray select 19;
_revivetime = paramsarray select 20;
_fullheal = if (paramsarray select 21 == 1) then {true} else {false};
_marker = if (paramsarray select 22 == 1) then {true} else {false};
_marker3d = if (paramsarray select 23 == 1) then {true} else {false};
...
AIS_MEDICAL_EDUCATION 		= 	_education; 	// Who can revive an unconscious unit? 0 == Everybody, 1 == Everybody with a First Aid Kit or Medkit, 2 == Only Medics (this affects both, AI and players!).
AIS_REVIVE_GUARANTY 		= 	_candie;		// If true you will fall everytime in uncoscious mode, regardless how strong the impact of damage was.
...
AIS_BLEEDOUT_TIME 			= 	_bleedouttime; 	// Basic life time in seconds until the unit bleed out and die.. The real life time depends on the real damage of the unit. (can be less or more time from the basic value)
AIS_REVIVETIME 				= 	_revivetime;	// Basic revive time in seconds. The real revive time depends on the real damage of the unit. (can be less or more time from the basic value)
AIS_STABILIZETIME 			= 	_revivetime;	// Basic stabilize time in seconds to stop the bleeding of a unconscious unit. The real revive time depends on the real damage of the unit. (can be less or more time from the basic value)
AIS_REVIVE_HEAL 			= 	_fullheal;		// If set to true the injured unit get completely healed after the revive. (casual gameplay without a medic)
...
AIS_SHOW_UNC_MARKERS	 	=	_marker; 		// If set to true, a marker will show injured units on the map.
AIS_SHOW_UNC_MESSAGE_TO 	= 	"Group"; 		// "None", "Side", "Group" --> who read the message about wounded units.
AIS_SHOW_UNC_3D_MARKERS 	=	_marker3d; 		// If set to true, an in-game visible 3D-icon shows you the position of injured units (within a range of 20 metres and 35 metres for medics).
